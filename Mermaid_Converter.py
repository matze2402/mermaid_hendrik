...
import pandas as pd
dict_of_csv_files = {}
for i in os.listdir("D:/Users/smhhborg(D_Laufwerk)/Documents/GitHub/Tests_for_link_ml_and_DCAT-ap/CSV_Files"):
    i_1 = i.replace(" ", ":").replace(".csv", "")
    with open(f"D:/Users/smhhborg(D_Laufwerk)/Documents/GitHub/Tests_for_link_ml_and_DCAT-ap/CSV_Files/{i}", 'r') as file:
        # Create a CSV reader object
        csv_file = pd.DataFrame(pd.read_csv(f"D:/Users/smhhborg(D_Laufwerk)/Documents/GitHub/Tests_for_link_ml_and_DCAT-ap/CSV_Files/{i}",delimiter=','))
        ...
        if i_1 == "dcat:Resource":
            ...
            for i_2 in test:
                # print(test[i_2])
                # print(f"{test[i_2][1]} {test[i_2][5]} [{test[i_2][2].replace('mandatory', '1').replace('recommended', '0').replace('optional', '1')}...{'*'if test[i_2][6] else '1'}] ")
                if test[i_2][4] == "-":
                ...
                # test_2.append(f"{test[i_2][1]} {test[i_2][5]} [{test[i_2][2].replace('mandatory', '1').replace('recommended', '0').replace('optional', '1')}...{'*'if test[i_2][6] else '1'}] ")
